...
//		String sql = "INSERT INTO BOARD ( " + " USER_SEQ, TITLE, CONTENT, REG_DT, READ_COUNT) "
//				+ " VALUES(?, ?, ?, now(), 0)";
...
//	}
	...
	public List<BoardDto> boardList(int limit, int offset) {
		String sql = "SELECT SQL_CALC_FOUND_ROWS A.* FROM ( " + "   SELECT b.BOARD_ID, " + "	      b.USER_SEQ, "
				+ "        u.USER_NAME, " + "        u.USER_PROFILE_IMAGE_URL, " + "        b.TITLE, "
				+ "        b.CONTENT, " + "        b.REG_DT, " + "        b.READ_COUNT " + "  FROM board b, user u "
				+ "  WHERE b.USER_SEQ = u.USER_SEQ " + "  ORDER BY BOARD_ID DESC " + "  ) A LIMIT ? OFFSET ?; ";
		return jdbcTemplate.query(sql, new BoardMapper(), limit, offset);
	}

	@Override
	public BoardDto boardDetail(int boardId) {
		String sql = "   SELECT b.BOARD_ID, " + "	      b.USER_SEQ, " + "        u.USER_NAME, "
				+ "        u.USER_PROFILE_IMAGE_URL, " + "        b.TITLE, " + "        b.CONTENT, "
				+ "        b.REG_DT, " + "        b.READ_COUNT " + "  FROM board b, user u "
				+ "  WHERE b.USER_SEQ = u.USER_SEQ " + "   AND b.BOARD_ID = ? ";
